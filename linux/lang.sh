...
# Install html-parsing (required by racket-langserver)
raco pkg install https://github.com/jeapostrophe/html-parsing.git
# Create html-parsing collection wrapper for racket-langserver compatibility
mkdir -p ~/.local/share/racket/9.2/collects/html-parsing
cat > ~/.local/share/racket/9.2/collects/html-parsing/main.rkt << 'EOF'
#lang racket/base
(require sxml/html)
(provide html->xexp)
EOF
# Install racket-langserver from git
cd /tmp
rm -rf racket-langserver
git clone https://github.com/6cdh/racket-langserver.git
cd racket-langserver
# Fix dependency names (racket-fixw instead of fixw)
sed -i 's/"fixw"/"racket-fixw"/' info.rkt
sed -i 's/"profile-flame-graph"//' info.rkt
raco pkg install --link .
cd ~
rm -rf /tmp/racket-langserver
